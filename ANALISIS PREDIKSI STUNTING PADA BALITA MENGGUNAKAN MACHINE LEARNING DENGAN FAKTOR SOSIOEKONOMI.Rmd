...
```{r}
# skenario 1 = default
svm_model_linear <-svm(stunting ~., data = data_train, kernel= 'linear')
svm_model_radial <-svm(stunting ~., data = data_train, kernel='radial')
svm_model_polynomial <-svm(stunting ~., data = data_train, kernel='polynomial')
svm_model_sigmoid <-svm(stunting ~., data = data_train, kernel='sigmoid')

# Prediksi
pred_linear <- predict(svm_model_linear, data_test)
pred_radial <- predict(svm_model_radial, data_test)
pred_poly <- predict(svm_model_polynomial, data_test)
pred_sigmoid <- predict(svm_model_sigmoid, data_test)

# Evaluasi
conf_matrix_linear <- confusionMatrix(pred_linear, data_test$stunting)
conf_matrix_radial <- confusionMatrix(pred_radial, data_test$stunting)
conf_matrix_poly <- confusionMatrix(pred_poly, data_test$stunting)
conf_matrix_sigmoid <- confusionMatrix(pred_sigmoid, data_test$stunting)

# Hasil
print(svm_model_linear)
print(svm_model_radial)
print(svm_model_polynomial)
print(svm_model_sigmoid)
...
```{r}
# Visualisasi confusion matrix
cm_svm_linear <- as.data.frame(conf_matrix_linear$table)
cm_svm_radial <- as.data.frame(conf_matrix_radial$table)
cm_svm_poly <- as.data.frame(conf_matrix_poly$table)
cm_svm_sigmoid <- as.data.frame(conf_matrix_sigmoid$table)

# Visualisasi
ggplot(cm_svm_linear, aes(x = Reference, y = Prediction, fill = Freq)) +
  geom_tile(color = "black") +
  geom_text(aes(label = Freq), size = 6) +
  scale_fill_gradient(low = "white", high = "skyblue") +
  labs(
    title = "Confusion Matrix - SVM Sigmoid",
    x = "Actual (Reference)",
    y = "Predicted",
    fill = "Count"
  ) +
...
```{r}
# skenario 2 = menambahkan parameter cost dan gamma
svm_model_linear2 <-svm(stunting ~., data = data_train,
                       kernel= 'linear',
                       cost = 10,
                       gamma = 0.1)
svm_model_radial2 <-svm(stunting ~., data = data_train,
                       ...
                       cost = 10,
                       gamma = 0.1)
svm_model_polynomial2 <-svm(stunting ~., data = data_train,
                           kernel='polynomial',
                           cost = 10,
                           gamma = 0.1)
svm_model_sigmoid2 <-svm(stunting ~., data = data_train,
                           kernel='sigmoid',
                           cost = 10,
                           gamma = 0.1)

# Prediksi
pred_linear2 <- predict(svm_model_linear2, data_test)
pred_radial2 <- predict(svm_model_radial2, data_test)
pred_poly2 <- predict(svm_model_polynomial2, data_test)
pred_sigmoid2 <- predict(svm_model_sigmoid2, data_test)

# Evaluasi
conf_matrix_linear2 <- confusionMatrix(pred_linear2, data_test$stunting)
conf_matrix_radial2 <- confusionMatrix(pred_radial2, data_test$stunting)
conf_matrix_poly2 <- confusionMatrix(pred_poly2, data_test$stunting)
conf_matrix_sigmoid2 <- confusionMatrix(pred_sigmoid2, data_test$stunting)


# Hasil
print(svm_model_linear2)
print(svm_model_radial2)
print(svm_model_polynomial2)
print(svm_model_sigmoid2)

print(conf_matrix_linear2)
print(conf_matrix_radial2)
print(conf_matrix_poly2)
print(conf_matrix_sigmoid2)
```
```{r}
# Visualisasi confusion matrix
# Ekstrak confusion matrix dari caret
cm_svm_linear2 <- as.data.frame(conf_matrix_linear2$table)
cm_svm_radial2 <- as.data.frame(conf_matrix_radial2$table)
cm_svm_poly2 <- as.data.frame(conf_matrix_poly2$table)
cm_svm_sigmoid2 <- as.data.frame(conf_matrix_sigmoid2$table)

# Visualisasi
ggplot(cm_svm_sigmoid2, aes(x = Reference, y = Prediction, fill = Freq)) +
  geom_tile(color = "black") +
  geom_text(aes(label = Freq), size = 6) +
  scale_fill_gradient(low = "white", high = "skyblue") +
  labs(
    title = "Confusion Matrix - SVM2 Sigmoid",
    x = "Actual (Reference)",
    y = "Predicted",
    fill = "Count"
  ) +
...
pred_polynomial3 <- predict(svm_model_polynomial3, data_test)
...
conf_matrix_poly3 <- confusionMatrix(pred_polynomial3, data_test$stunting)
...
# Visualisasi confusion matrix
# Ekstrak confusion matrix dari caret
cm_svm_linear3 <- as.data.frame(conf_matrix_linear3$table)
cm_svm_radial3 <- as.data.frame(conf_matrix_radial3$table)
cm_svm_poly3 <- as.data.frame(conf_matrix_poly3$table)
cm_svm_sigmoid3 <- as.data.frame(conf_matrix_sigmoid3$table)

# Visualisasi
ggplot(cm_svm_sigmoid3, aes(x = Reference, y = Prediction, fill = Freq)) +
  geom_tile(color = "black") +
  geom_text(aes(label = Freq), size = 6) +
  scale_fill_gradient(low = "white", high = "skyblue") +
  labs(
...
metrics_rf <- bind_rows(
  tibble(Model = "ntree 1000",
         Metric = c("Recall", "Precision", "F1", "Accuracy"),
         Value = c(conf_matrix_rf1a$byClass[c("Precision", "Recall", "F1")],
                   conf_matrix_rf1a$overall["Accuracy"])),
  tibble(Model = "ntree 500",
         Metric = c("Recall", "Precision", "F1", "Accuracy"),
         Value = c(conf_matrix_rf1b$byClass[c("Precision", "Recall", "F1")],
                   conf_matrix_rf1b$overall["Accuracy"])),
  tibble(Model = "ntree 100",
         Metric = c("Recall", "Precision", "F1", "Accuracy"),
         Value = c(conf_matrix_rf1c$byClass[c("Precision", "Recall", "F1")],
                   conf_matrix_rf1c$overall["Accuracy"]))
)
...
# Visualisasi
# Gabungkan semua metrik ke dalam satu tibble
metrics_rf2 <- bind_rows(
  tibble(Model = "ncol",
         Metric = c("Recall", "Precision", "F1", "Accuracy"),
         Value = c(conf_matrix_rf2a$byClass[c("Precision", "Recall", "F1")],
                   conf_matrix_rf2a$overall["Accuracy"])),
  tibble(Model = "sqrt",
         Metric = c("Recall", "Precision", "F1", "Accuracy"),
         Value = c(conf_matrix_rf2b$byClass[c("Precision", "Recall", "F1")],
                   conf_matrix_rf2b$overall["Accuracy"])),
  tibble(Model = "log2",
         Metric = c("Recall", "Precision", "F1", "Accuracy"),
         Value = c(conf_matrix_rf2c$byClass[c("Precision", "Recall", "F1")],
                   conf_matrix_rf2c$overall["Accuracy"]))
)

# Visualisasi 
ggplot(metrics_rf2, aes(x = Metric, y = Value, fill = Model)) +
  geom_bar(stat = "identity", position = position_dodge(0.8), width = 0.7) +
  geom_text(aes(label = round(Value, 3)), 
            position = position_dodge(0.8),
            vjust = -0.5, 
...
```{r}
# Skenario 1
metrics_svm_default <- bind_rows(
  tibble(Model = "Linear",
         Metric = c("Precision", "Recall", "F1", "Accuracy"),
         Value = c(conf_matrix_linear$byClass[c("Precision", "Recall", "F1")],
  ...
  tibble(Model = "Polynomial",
         Metric = c("Precision", "Recall", "F1", "Accuracy"),
         Value = c(conf_matrix_poly$byClass[c("Precision", "Recall", "F1")],
                   conf_matrix_poly$overall["Accuracy"])),
  tibble(Model = "Sigmoid",
         Metric = c("Precision", "Recall", "F1", "Accuracy"),
         Value = c(conf_matrix_sigmoid$byClass[c("Precision", "Recall", "F1")],
                   conf_matrix_sigmoid$overall["Accuracy"])))

# Visualisasi 
ggplot(metrics_svm_default, aes(x = Metric, y = Value, fill = Model)) +
  geom_bar(stat = "identity", position = position_dodge(0.8), width = 0.7) +
  geom_text(aes(label = round(Value, 3)), 
            position = position_dodge(0.8),
            vjust = -0.5, 
...
```{r}
# Skenario 2
metrics_svm2 <- bind_rows(
  tibble(Model = "Linear",
         Metric = c("Precision", "Recall", "F1", "Accuracy"),
         Value = c(conf_matrix_linear2$byClass[c("Precision", "Recall", "F1")],
                   conf_matrix_linear2$overall["Accuracy"])),
  tibble(Model = "Radial",
         Metric = c("Precision", "Recall", "F1", "Accuracy"),
  ...
  tibble(Model = "Polynomial",
         Metric = c("Precision", "Recall", "F1", "Accuracy"),
         Value = c(conf_matrix_poly2$byClass[c("Precision", "Recall", "F1")],
                   conf_matrix_poly2$overall["Accuracy"])),
  tibble(Model = "Sigmoid",
         Metric = c("Precision", "Recall", "F1", "Accuracy"),
         Value = c(conf_matrix_sigmoid2$byClass[c("Precision", "Recall", "F1")],
                   conf_matrix_sigmoid2$overall["Accuracy"])))

# Visualisasi 
ggplot(metrics_svm2, aes(x = Metric, y = Value, fill = Model)) +
  geom_bar(stat = "identity", position = position_dodge(0.8), width = 0.7) +
  geom_text(aes(label = round(Value, 3)), 
            position = position_dodge(0.8),
            vjust = -0.5, 
            size = 3) +
  labs(title = "Perbandingan Model SVM 2",
       y = "Nilai Metrik",
       x = "Metrix Evaluasi",
       fill = "Kernel") +
  theme_minimal()
...
metrics_svm2 <- bind_rows(
  tibble(Model = "Linear",
         Metric = c("Precision", "Recall", "F1", "Accuracy"),
         Value = c(conf_matrix_linear3$byClass[c("Precision", "Recall", "F1")],
                   conf_matrix_linear3$overall["Accuracy"])),
  tibble(Model = "Radial",
         Metric = c("Precision", "Recall", "F1", "Accuracy"),
         Value = c(conf_matrix_radial2$byClass[c("Precision", "Recall", "F1")],
                   conf_matrix_radial2$overall["Accuracy"])),
