...
```
```{r}
# nilai asli (ground truth)
nilai_asli_rf <- data_test$stunting

df_rf <- data.frame(Nilai_Asli = nilai_asli_rf, Nilai_Prediksi = pred_rf)

# Perbandingan Nilai Prediksi Random Forest dengan Nilai Asli
ggplot(df_rf, aes(x = Nilai_Asli, y = Nilai_Prediksi)) +
  geom_jitter(width = 0.2, height = 0.2, color = "blue", size = 2) +
  labs(title = "Perbandingan Nilai Prediksi Random Forest dengan Nilai Asli",
       x = "Nilai Asli (0 = Stunting Rendah, 1 = Stunting Tinggi)",
       y = "Nilai Prediksi Random Forest") +
  theme_minimal()
```
...
# Hasil model svm-karnel polynomial
print(svm_poly)
```
```{r}
# Menggunakan karnel linear
svm_linear <- svm(stunting ~ ., data = data_train, kernel = "linear", cost = 100, gamma = 0.1)

# Hasil model svm-karnel linear
print(svm_linear)
```
```{r}
...
# Hasil model svm-karnel sigmoid
print(svm_sigmoid)
```
...
ggplot(importance_df, aes(x = reorder(Feature, Importance), y = Importance)) +
  geom_bar(stat = "identity", fill = "blue") +
  coord_flip() +
  labs(title = "Feature Importance pada Model SVM",
       x = "Faktor Sosioekonomi",
       y = "Tingkat Kepentingan") +
  theme_minimal()

# Ambil koefisien dari model svm_radial fine tunning
coefs_radial <- t(svm_radial$coefs) %*% svm_radial$SV
importance_radial <- as.vector(coefs_radial)
features <- names(data_train)[-which(names(data_train) == "stunting")]

# Buat dataframe untuk visualisasi
importance_radial_df <- data.frame(
  Feature = features,
  Importance = abs(importance_radial)
)

# Visualisasi
ggplot(importance_radial_df, aes(x = reorder(Feature, Importance), y = Importance)) +
  geom_bar(stat = "identity", fill = "darkgreen") +
  coord_flip() +
  labs(title = "Feature Importance pada Model SVM\ndengan Kernel Radial Fine Tunning",
       x = "Faktor Sosioekonomi",
       y = "Tingkat Kepentingan") +
  theme_minimal()
```
...
```{r}
# Melakukan prediksi
pred_radial <- predict(svm_radial, data_test)

# Evaluasi model pada test data
conf_matrix_svmradial <- confusionMatrix(pred_radial, data_test$stunting)
conf_matrix_svmradial
```
```{r}
# nilai asli (ground truth)
nilai_asli_svm <- data_test$stunting

df_svm <- data.frame(Nilai_Asli = nilai_asli_svm, Nilai_Prediksi = pred_radial)

# Perbandingan Nilai Prediksi Random Forest dengan Nilai Asli
ggplot(df_svm, aes(x = Nilai_Asli, y = Nilai_Prediksi)) +
  geom_jitter(width = 0.2, height = 0.2, color = "blue", size = 2) +
  labs(title = "Perbandingan Nilai Prediksi SVM dengan Nilai Asli",
       x = "Nilai Asli (0 = Stunting Rendah, 1 = Stunting Tinggi)",
       y = "Nilai Prediksi SVM") +
  theme_minimal()
```
