...
## b. Melakukan Bootstrap sampling
```{r}
...
# Set seed untuk hasil yang reproducible
set.seed(123)
...
##SVM
```{r}
final_data$stunting <- as.factor(final_data$stunting)

set.seed(101)
sample <- sample.split(final_data$stunting, SplitRatio = 0.7) # 70% untuk data train dan sisanya untuk test
...
```{r}
tune.out <- tune(svm, stunting ~ ., data = final_data, ranges = list(cost = c(0.1, 1, 10), gamma = c(0.1, 1, 10)))
summary(tune.out)
```
```{r}
colnames(train)
...
# Model
```{r}
model <- svm(stunting ~., data = train[1:7])
summary(model)
```


```{r}
predict.values <- predict(model,test[1:7])
table(predict.values, test$stunting)
```
Interpretasi:
1. Baris pertama (0) menunjukkan situasi ketika sebenarnya adalah kelas 0 (Tidak Stunting). Model berhasil memprediksi dengan benar sebagai kelas 0 sebanyak 8126 kali (True Negative - TN). Model gagal sebanyak 303 kali (False Positive - FP), di mana balita yang sebenarnya tidak stunting justru diprediksi stunting.

2. Baris kedua (1) menunjukkan situasi ketika sebenarnya adalah kelas 1 (Stunting). Model berhasil memprediksi dengan benar sebagai kelas 1 sebanyak 8299 kali (True Positive - TP). Model gagal sebanyak 591 kali (False Negative - FN), di mana balita yang sebenarnya stunting justru diprediksi tidak stunting.

3. Kesimpulan: Model SVM memiliki kinerja baik dalam memprediksi kelas 0 dan 1. Namun diperlukan peninjauan ulang atau peningkatan model untuk meningkatkan kemampuan dalam memprediksi kelas.

# Model fine Tuning

```{r}
tuned.svm <-svm(stunting ~., data=train[1:7], kernel='radial', cost =100, gamma=0.1)
predicted.values <- predict(tuned.svm, test[1:7])
table(predicted.values, test$stunting)
```
Interpretasi:
1. Baris pertama (0) menunjukkan situasi ketika sebenarnya adalah kelas 0. Model berhasil memprediksi dengan benar sebagai kelas 0 sebanyak 8570 kali, dan gagal sebanyak 147 kali.

2. Baris kedua (1) menunjukkan situasi ketika sebenarnya adalah kelas 1. Model hanya berhasil memprediksi dengan benar sebagai kelas 1 sebanyak 8602 kali, sedangkan seharusnya adalah 147 kali.
